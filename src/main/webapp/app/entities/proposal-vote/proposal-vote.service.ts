...
export class ProposalVoteService {
  ...
  protected convertDateFromClient(proposalVote: IProposalVote): IProposalVote {
    const copy: IProposalVote = Object.assign({}, proposalVote, {
      creationDate: proposalVote.creationDate && proposalVote.creationDate.isValid() ? proposalVote.creationDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((proposalVote: IProposalVote) => {
        proposalVote.creationDate = proposalVote.creationDate ? moment(proposalVote.creationDate) : undefined;
      });
    }
    return res;
  }
