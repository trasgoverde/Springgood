...
export class AlbumService {
  ...
  protected convertDateFromClient(album: IAlbum): IAlbum {
    const copy: IAlbum = Object.assign({}, album, {
      creationDate: album.creationDate && album.creationDate.isValid() ? album.creationDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((album: IAlbum) => {
        album.creationDate = album.creationDate ? moment(album.creationDate) : undefined;
      });
    }
    return res;
  }
