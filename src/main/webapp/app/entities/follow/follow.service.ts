...
export class FollowService {
  ...
  protected convertDateFromClient(follow: IFollow): IFollow {
    const copy: IFollow = Object.assign({}, follow, {
      creationDate: follow.creationDate && follow.creationDate.isValid() ? follow.creationDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((follow: IFollow) => {
        follow.creationDate = follow.creationDate ? moment(follow.creationDate) : undefined;
      });
    }
    return res;
  }
