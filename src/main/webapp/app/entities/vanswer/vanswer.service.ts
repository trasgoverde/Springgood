...
export class VanswerService {
  ...
  protected convertDateFromClient(vanswer: IVanswer): IVanswer {
    const copy: IVanswer = Object.assign({}, vanswer, {
      creationDate: vanswer.creationDate && vanswer.creationDate.isValid() ? vanswer.creationDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((vanswer: IVanswer) => {
        vanswer.creationDate = vanswer.creationDate ? moment(vanswer.creationDate) : undefined;
      });
    }
    return res;
  }
