...
export class CmessageUpdatePage {
  ...
  cmessageTextInput = element(by.id('field_cmessageText'));
  ...
  async setCmessageTextInput(cmessageText: string): Promise<void> {
    await this.cmessageTextInput.sendKeys(cmessageText);
  }

  async getCmessageTextInput(): Promise<string> {
    return await this.cmessageTextInput.getAttribute('value');
  }
